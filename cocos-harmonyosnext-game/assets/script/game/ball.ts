...
export class Ball extends Component {
    ...
    onDestroy() {
        ...
        if (!Constants.game || !Constants.game.node) {
            return;  
        }
        ...
        if (Constants.game.state === Constants.GAME_STATE.PLAYING) {
            ...
            if (this.jumpState === Constants.BALL_JUMP_STATE.SPRINT) {
                ...
                for (let i = 0; i < Constants.DIAMOND_NUM; i++) {
                    if (Math.abs(this.node.position.y - diamondSprintList[i].position.y) <= Constants.DIAMOND_SPRINT_SCORE_AREA && 
                        Math.abs(this.node.position.x - diamondSprintList[i].position.x) <= Constants.DIAMOND_SPRINT_SCORE_AREA) {
                ...
                for (let i = this.currBoardIdx + 1; i >= 0; i--) {
                    ...
                    if (Math.abs(pos.x - boardPos.x) <= boardList[i].getRadius() && 
                        Math.abs(pos.y - (boardPos.y + Constants.BOARD_HEIGTH)) <= Constants.DIAMOND_SCORE_AREA) {
                        boardList[i].checkDiamond(pos.x);
                    }

                    if (this.jumpState === Constants.BALL_JUMP_STATE.FALLDOWN) {
                        if (this.currJumpFrame > Constants.PLAYER_MAX_DOWN_FRAMES || 
                            (this.currBoard.node.position.y - pos.y) - (Constants.BOARD_GAP + Constants.BOARD_HEIGTH) > 0.001) {
    ...
    activeCurrBoard() {
        ...
        if (!this.currBoard.isActive) {
            ...
            if (boardType !== Constants.BOARD_TYPE.NORMAL && boardType !== Constants.BOARD_TYPE.DROP || 
                Math.abs(pos.x - boardPos.x) <= Constants.BOARD_RADIUS_CENTER) {
    ...
    setPosY() {
        ...
        if (this.jumpState === Constants.BALL_JUMP_STATE.JUMPUP) {
            ...
            if (this.currJumpFrame >= Constants.DIAMOND_START_FRAME + 20 && 
                this.currJumpFrame <= Constants.BALL_JUMP_FRAMES_SPRINT - 50 && 
                Math.floor(this.currJumpFrame) % Math.floor(Constants.DIAMOND_SPRINT_STEP_Y / Constants.BALL_JUMP_STEP_SPRINT) == 0) {
